...
from utils import AverageMeter, vprint, get_target_hr, goal_oriented_loss, AttackDataset, PartialDataLoader
...
class FLOJOAttacker(BasicAttacker):
    def __init__(self, attacker_config):
        ...
        self.step_item = attacker_config['step_item']
    ...
    def train_fake(self, surrogate_model, surrogate_trainer, fake_tensor, adv_opt,
        ...
        with higher.innerloop_ctx(surrogate_model, opt) as (fmodel, diffopt):
            ...
            for s in range(self.look_ahead_step):
                ...
                score_p = (scores * profiles).sum(dim=1).detach()
                ...
                loss_n = (loss_n * normed_n_profiles).sum()
                diffopt.step(loss_p + loss_n)
                loss_p = loss_p / temp_fake_user_tensor.shape[0]
                loss_n = loss_n / temp_fake_user_tensor.shape[0]
    ...
    def init_fake_tensor(self, temp_fake_user_tensor):
        ...
        fake_tensor[:, self.target_item_tensor] = 10.
        for u_idx in range(temp_fake_user_tensor.shape[0]):
            ...
            fake_tensor[u_idx, torch.tensor(filler_items)] = -np.inf
        fake_tensor.requires_grad = True
        return fake_tensor

    def add_filler_items(self, fake_tensor, temp_fake_user_tensor):
        _, filler_items = fake_tensor.topk(self.step_item, dim=1)
        filler_items = filler_items.cpu().numpy().tolist()
        for u_idx in range(temp_fake_user_tensor.shape[0]):
            f_u = temp_fake_user_tensor[u_idx]
            self.fake_user_inters[f_u - self.n_users] += filler_items[u_idx]
            self.dataset.train_data[f_u] |= set(filler_items[u_idx])
            self.dataset.train_array += [[f_u, item] for item in filler_items[u_idx]]
        ...
        for retraining_epoch in range(self.n_retraining_epochs):
            ...
            vprint('Retraining Epoch {:d}/{:d}, Time: {:.3f}s, Added items: {:d}'
                   ...
                   format(retraining_epoch, self.n_retraining_epochs, consumed_time,
                          self.step_item * (retraining_epoch + 1), t_loss, target_hr * 100.), verbose)
    ...
    def generate_fake_users(self, verbose=True, writer=None):
        ...
        for i_step in range(1, len(fake_user_end_indices)):
            ...
            self.dataset.train_data += [{} for _ in range(n_temp_fakes)]
