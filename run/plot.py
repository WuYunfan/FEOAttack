...
def main():
    ...
    fig, (ax11, ax12, ax13) = (
        plt.subplots(nrows=1, ncols=3, constrained_layout=True, figsize=(20, 4)))
    ...
    ax13.scatter(recall, attack, color='#7570A0', edgecolor=darken_color('#7570A0', 0.2), s=50)
    names = [f'$w_k$\n={w}' for w in [0, 0.0001, 0.001, 0.01, 0.1, 1]]
    for xi, yi, name in zip(recall, attack, names):
        ax13.text(xi - 0.005, yi, name, fontsize=12, ha='left', va='bottom', color='#7570A0')
    attack = np.array([0.299, 0.419, 0.582, 0.647, 0.708, 0.553, 0.559])
    recall = np.array([0.022, 0.005, 0.017, 0.047, 0.395, 0.296, 0.071])
    ax13.scatter(recall, attack, color='#F7B76D', edgecolor=darken_color('#F7B76D', 0.2), s=50)
    names = ['Random', 'Bandwagon', 'RevAdv', 'DPA2DL', 'RAPU-R', 'Leg-UP', 'UBA']
    for xi, yi, name in zip(recall, attack, names):
        ax13.text(xi - 0.005, yi, name, fontsize=12, ha='left', va='bottom', color='#F7B76D')
    ax13.set_ylim(0.2, 1.5)
    ...
    pdf = PdfPages('hyper-gowalla-2.pdf')
    base_mean = [0.787]
    ...
    pdf = PdfPages('hyper-amazon-2.pdf')
    base_mean = [0.399]
